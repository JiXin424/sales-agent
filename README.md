...
| 日期 | 摘要 |
|------|------|
| [2026-07-08](changelog/2026-07-08.md) | **新增「场景教练」（Scenario Coach，feature flag 默认关）: 预设销售场景问答拦截**。新自包含包 `src/sales_agent/scenarios/`（`data/销冠智慧教练手册.md` 6 场景 11 预设问题 + md→`ScenarioRegistry` loader + LLM 11选1+none 匹配 + 预设答案）。Online Graph `normalize_turn` 后插 `scenario_coach` 节点：用户问题命中某预设场景（confidence≥0.8）→ 直接返回预设 `answer_dict`（文末带 `《销冠智慧教练手册·2026年4月版》` 来源，`source_type=scenario_coach→教练手册` 标签）→ `log_scenario_response`→END，跳过 RAG/生成；未命中→透传原路径行为零变化。新增 `ScenarioCoachConfig{enabled=false, confidence_threshold=0.8}`，env `SCENARIO_COACH_ENABLED`（bool）/ `SCENARIO_COACH_CONFIDENCE_THRESHOLD`（float，repo 首个 float-from-env 覆盖）。`scenario_coach_enabled` 注入**两个** input_state 构造点（HTTP `online_conversation` + 钉钉 stream `graph_stream`，覆盖 lessons #35 stream 入口）；`route_online_message` 仅开启且 chat/direct_chat 时改路。matcher/loader/LLM 全链路 fail-open 放行（lessons #34）。无 DB、无 Alembic、内容随镜像分发 → 实例 A 开/B 关、prod3 部署只改 `secrets/<tenant>.env`。176 图单测 + scenario 全测通过；生产验证待 `docker logs sales-agent-<tenant>-stream` |
