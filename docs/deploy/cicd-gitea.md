...
> - **fan-out 2 台**：prod3(本地) + test `47.118.16.235`（deploy-release）；**prod2 `47.120.50.181` 降级为开发机**，手动 `deploy-release.sh` 管理，不在 fan-out。
...
└──┬────────────────┬───────────────────┬────────────────────┘
 ...
 主控自己           本机 47.120.55.219     test 47.118.16.235(cn-test)
...
- **test 47.118.16.235(跨地域)**:跑 sales-agent(deploy-release,隔离端口 8002,DingTalk 关)。纯运行时。

## 2. 核心设计:`registry.internal` 分流

Docker tag 内嵌 registry 主机名;test跨地域只能走公网、其余走私网,**主机名不同无法共用 tag**。解法:用名字 `registry.internal`,各机 `/etc/hosts` 解析到自己能到的 IP:

| 机器 | /etc/hosts 里 registry.internal → |
|------|----------------------------------|
| 主控 | `127.0.0.1` |
| 本机(同 VPC) | `172.25.186.209`(主控私网) |
| test(跨地域) | `47.120.50.181`(主控公网) |
...
- test 47.118.16.235:私网 172.18.128.88,cn-**test**,**跨地域,私网不通主控**,只能走公网。
- **test拉镜像要主控两层防火墙都放行 TCP:5000 来源 47.118.16.235**:
...
- registry 走 **HTTP + htpasswd**，无 TLS（已移除 domain.crt/key、registry-ca.crt）。
...
**各目标机 `/root/code/sales-agent/secrets/taishan.env`:** 真实凭证(MODEL_API_KEY、DingTalk 等)。**DingTalk 在主控/test关闭**(避免与生产 stream 重复回复),只有本机生产开着。
...
| method | 脚本 | 用于 | 干什么 |
|--------|------|------|--------|
| `deploy-release` | `scripts/deploy-release.sh --yes` | 主控、test | `REGISTRY_IMAGE` pull → 渲染 compose(`scripts/render-multitenant-deploy.py`,image 走 `OVERRIDE_IMAGE` env 注入 sha)→ `docker compose up -d` → 健康检查 |
...
2. `deploy-fanout`:`IMAGE=...:<sha> scripts/ci-fanout.sh` 部署 **2 台**（prod3 本地 + test）。
...
1. **fan-out 的 `ssh` 必须 `ssh -n`**(`scripts/ci-fanout.sh`、`scripts/sync-code.sh`)。否则 `while read` 循环里的 ssh 吸走清单文件的 stdin,循环提前结束、**跳过第一个 SSH 之后的目标**(曾导致test被跳过、一直不更新)。
...
4. **test跨地域要 SG + ufw 两层都开 :5000**(见 §3)。
...
- 三台 `docker inspect sales-agent-<id>-api --format '{{.Config.Image}}'` == 最新 sha(主控/test)或 sales-agent:latest(本机)。
- 三台 `/health` 200、`/ready` 200。
- 本机 `curl -k https://qiyelongxia.com.cn/` 200(生产 traefik 未动)。
- test `docker pull registry.internal:5000/sales-agent:smoke` 成功(跨地域公网+TLS+CA+SG+ufw 全对)。
...
- **镜像**：CI `build-and-push` mirror `docker.1ms.run/neo4j:5` → `registry.internal:5000/neo4j:5`（test跨地域稳定拉取）。
- **每台运维手动步骤**：
...
test及未来不给源码的目标机用 **deploy 镜像** 部署（不 git sync、不放源码）：
...
- **test**：`deploy-targets.json` method=`image-deploy` env=`test`；`tenants.test.json` 含 fuduoduo（8103）。
