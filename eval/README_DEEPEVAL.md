...
Sales Agent + DeepEval 集成 —— 直接走 Online Graph（与钉钉用户相同的生产代码路径）+ LLM-as-a-Judge 自动评分。
...
# === Agent 模型（评估脚本自己调 invoke_online_turn）===
...
| 文件 | 用途 |
|------|------|
| `deepeval_eval.py` | **主评估脚本** — 加载问题 → 调 invoke_online_turn → 跑全部指标 → 输出 4 格式报告 |
| `deepeval_metrics.py` | **指标工厂** — 11 个指标的构建函数 + `get_metrics_for_question()` 自动组合 |
| `deepeval_test_cases.py` | **管道调用** — `call_agent_pipeline()` 直连 invoke_online_turn，`build_llm_test_case()` |
...
> **混合策略（Ontology 为主 + RAG 兜底）也自动适用**。评估只看 `invoke_online_turn()` 的输出，不关心内部怎么选择检索路径。
...
A: **不需要**。评估只看 `invoke_online_turn()` 的最终输出，内部策略自动跟随。
