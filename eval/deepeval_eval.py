#!/usr/bin/env python3
"""
Sales Agent DeepEval 评估脚本 —— 黑盒端到端评估 + LLM-as-a-Judge 自动评分。

功能:
  1. 从 questions.md + ground_truth_30q.json 加载问题
  2. 调 Agent /eval/streaming-chat API (模拟钉钉流式)
  3. DeepEval 指标自动评分 + Arena 盲测
  4. 每 5 题写 checkpoint, 支持 --resume 恢复
  5. 输出 JSON / Markdown / CSV / HTML 四格式报告 + TTFT + Token

用法:
  python eval/deepeval_eval.py --app-url http://localhost:8003 --limit 5
  python eval/deepeval_eval.py --app-url-legacy http://localhost:8003 --app-url-ontology http://localhost:8004
  python eval/deepeval_eval.py ... --resume
"""
...
from deepeval_test_cases import (
    QuestionItem, build_llm_test_case, call_agent_api, load_all_questions,
)
...
class SingleResult:
    ...
    answer: str; task_type: str; risk_level: str; sources_count: int
...
def _write_checkpoint(path, kb_label, model, results, questions, skip_ids):
    data = {
        ...
        "results": [{
            ...
            "answer": r.answer[:500], "latency_ms": r.latency_ms, "ttft_ms": r.ttft_ms,
...
def _load_checkpoint(path):
    ...
    for rd in data.get("results", []):
        results.append(SingleResult(
            ...
            answer=rd.get("answer",""), task_type="", risk_level="",
            sources_count=0, latency_ms=rd.get("latency_ms",0), ttft_ms=rd.get("ttft_ms",0),
...
# ── 主评估 ──
async def run_eval(app_url, kb_label, questions, model=None, tenant_id="taishan",
    ...
    async def process_one(q):
        async with semaphore:
            api = await call_agent_api(app_url, q, model=model, tenant_id=tenant_id)
            ...
            sr = SingleResult(
                ...
                answer=api.answer_text[:3000], task_type=api.task_type,
                risk_level=api.risk_level, sources_count=len(api.sources),
...
def write_json_report(results, summaries, questions, meta, path):
    ...
    rep = {
        ...
        "details": [{
            ...
            "answer": r.answer, "task_type": r.task_type, "risk_level": r.risk_level,
...
def write_csv_report(results, path):
    ...
    hdr = ["question_id","question","kb_label","model","task_type","risk_level",
           ...
           "total_tokens","answer","error"] + all_m
    with open(path, "w", newline="", encoding="utf-8-sig") as f:
        ...
        for r in results:
            w.writerow([r.question_id, r.question, r.kb_label, r.model,
                        ...
                        r.total_tokens, r.answer, r.error]
...
# ── CLI ──
def main():
    p = argparse.ArgumentParser(description="Sales Agent DeepEval 评估")
    p.add_argument("--app-url", default=None)
    p.add_argument("--app-url-legacy", default=None)
    p.add_argument("--label-legacy", default="legacy_rag")
    p.add_argument("--app-url-ontology", default=None)
    p.add_argument("--label-ontology", default="ontology_neo4j")
    p.add_argument("--models", default="all")
    p.add_argument("--tenant-id", default="taishan")
    p.add_argument("--tenant-id-legacy", default=None)
    p.add_argument("--tenant-id-ontology", default=None)
    ...
    targets = []
    tl = args.tenant_id_legacy or args.tenant_id
    to = args.tenant_id_ontology or args.tenant_id
    if args.app_url: targets.append((args.app_url, "eval", args.tenant_id))
    if args.app_url_legacy: targets.append((args.app_url_legacy, args.label_legacy, tl))
    if args.app_url_ontology: targets.append((args.app_url_ontology, args.label_ontology, to))
    if not targets: print("[ERROR] 至少指定一个 --app-url", file=sys.stderr); sys.exit(1)

    models = _parse_models(args.models)
    all_results = []

    for app_url, label, tenant_id in targets:
        for model in models:
            md = model or "(default)"
            ...
            print(f"\n{'='*60}")
            print(f"[EVAL] {label} | {md} | {app_url} | tenant={tenant_id}")
            print(f"[EVAL] {remaining}/{len(questions)} questions")
            print(f"{'='*60}")

            t0 = time.monotonic()
            results = asyncio.run(run_eval(
                app_url=app_url, kb_label=label, questions=questions,
                model=model, tenant_id=tenant_id, concurrency=args.concurrency,
    ...
    meta = {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "total_questions": len(questions),
        "targets": [{"url": u, "label": l, "tenant": t} for u,l,t in targets],
            ...
            for j in range(i+1, len(kb_labels)):
                ...
                for qid, kbs in by_qid.items():
                    ...
                    arena = run_arena_comparison(
                        kb_a_label=ka, kb_a_answer=ra.answer[:2000],
                        kb_b_label=kb, kb_b_answer=rb.answer[:2000],
