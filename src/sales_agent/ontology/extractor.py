...
async def extract_facts(
    ...
    """按实体分批抽取事实。每批使用完整文档上下文（自动分片适配）。"""
    # 将文档切为可用片段：短文档用全文，长文档用首片（包含最完整上下文）
    chunks = _chunk_content(content)
    ...
    for i in range(0, len(entities), batch_size):
        ...
        raw = await _generate_with_retry(
            chat_model,
            messages=[{"role": "user", "content": FACT_EXTRACTION_PROMPT.format(content=primary_chunk, entities_json=entities_json)}],
            temperature=0.1,
            max_tokens=6000,
        )
