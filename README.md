...
- 抽取链修复（2026-07-11）：`prompts.task.sales_action_extractor` 抽取器 prompt 在 1555b5e「prompt 迁 YAML」重构中遗漏、且 `parser.py` 未 import `get_prompt`，导致启用后必崩、未启用时模型「幻觉」承诺提醒。现已补齐 YAML prompt + import，并把 `_build_messages` 移入 try 兑现「失败即降级」契约。灰度启用中：CI/CD 连的 5 租户（taishan/taishankaifa2/taishanyanshi/songbai/fuduoduo）。
...
| 日期 | 摘要 |
|------|------|
| [2026-07-11](changelog/2026-07-11.md) | **销售动作提醒抽取链修复 + 灰度启用**：诊断「5分钟后提醒我上厕所→机器人只幻觉答应、到点无提醒」。根因非 bug 而是功能断链+默认关：`sales_action_extractor` prompt 在 1555b5e「prompt 迁 YAML」重构中遗漏、`parser.py` 未 import `get_prompt` 且调用在 try 外→启用即崩；默认 `enabled=false`→未启用时落普通 chat，模型幻觉承诺。修复：补 `prompts.task.sales_action_extractor`、加 import、`_build_messages` 移入 try 兑现降级契约、新增 `SALES_ACTIONS_ENABLED` 环境变量覆盖。灰度到 CI/CD 5 租户（taishan/taishankaifa2/taishanyanshi/songbai/fuduoduo），门控默认关、按租户 .env 开。无 DB 变更（0016 已够）。62 sales_actions 单测 + 27 loader/config 测试通过。 |
