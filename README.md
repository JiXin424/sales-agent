...
- 默认关闭：`sales_actions.enabled=false`；可用环境变量 `SALES_ACTIONS_ENABLED=true` 按租户单独打开（覆盖优先于 YAML）。后台调度器由 `sales_actions.scheduler_enabled`（默认 true，仅 enabled 时生效）控制，随 worker 启动。默认时区 `Asia/Shanghai`，digest 窗口 09:00 / 18:30。
- 抽取链修复 + 全量启用（2026-07-11）：`prompts.task.sales_action_extractor` 抽取器 prompt 在 1555b5e「prompt 迁 YAML」重构中遗漏、且 `parser.py` 未 import `get_prompt`，导致启用后必崩、未启用时模型「幻觉」承诺提醒。金丝雀又连修两个 bug：抽取器 `now` 用 UTC 却声明 Asia/Shanghai 时区，LLM 把相对时间算成 8 小时前被判 `past_time`（`_build_messages` 现把 now 换算到声明时区）；跨轮澄清合并 `_merge_partial` 的 `missing_fields ... or old` 会复活旧缺失字段，导致 title 已给出仍被判 `missing_title`（现按合并后实际值重算 + 完整新请求旁路陈旧 partial）。**已在 CI/CD 连的 5 租户全量启用并验证**（taishan/taishankaifa2=prod2、taishanyanshi/songbai=prod3、fuduoduo=test）；taishan 真实钉钉端到端验证通过（建库→模板回执→worker 调度器到点→卡片投递 success）。另修投递卡片「一直加载中」：提醒卡片是钉钉 AI 流式卡片，scheduler 只 `createAndDeliver` 不发 `streaming_finalize` 结束帧 → 永远转圈；`_deliver_single`/`_deliver_digest` 现补 finalize 定格内容。再修跨租户「串台」：共享库租户对（prod2、prod3）各跑一个 worker 扫同一张表，`claim_due_reminders` 原不按 tenant 过滤 → A 租户 worker 抢走并用自己钉钉凭证投递 B 租户提醒；现把每容器 `TENANT_ID` 串进调度器，认领/digest/pursuit 三个 pass 均按租户收敛。
...
用户在互动卡片上的点赞 / 点踩 / 问题反馈多选，经钉钉 Stream 长连接回推。stream client 注册卡片回调 topic（`/v1.0/card/instances/callback`），`SalesAgentCardCallbackHandler` 防御式采集，原始载荷落表 `dingtalk_card_callback_events`（migration 0018）。
...
| 日期 | 摘要 |
|------|------|
| [2026-07-11](changelog/2026-07-11.md) | **销售动作提醒抽取链修复 + 全量启用**：诊断「5分钟后提醒我上厕所→机器人只幻觉答应、到点无提醒」。根因非 bug 而是功能断链+默认关：`sales_action_extractor` prompt 在 1555b5e「prompt 迁 YAML」重构中遗漏、`parser.py` 未 import `get_prompt` 且调用在 try 外→启用即崩；默认 `enabled=false`→未启用时落普通 chat，模型幻觉承诺。修复：补 `prompts.task.sales_action_extractor`、加 import、`_build_messages` 移入 try、新增 `SALES_ACTIONS_ENABLED` 环境变量覆盖。灰度金丝雀又连修两个 bug：①抽取器 `now` 用 UTC 却声明 Asia/Shanghai→LLM 把相对时间算成 8h 前判 `past_time`（now 换算到声明时区）；②跨轮合并 `_merge_partial` 的 `missing_fields ... or old` 复活旧缺失→title 已给出仍判 `missing_title`（按合并后实际值重算 + 完整新请求旁路陈旧 partial）。**CI/CD 连的 5 租户全量启用并验证**（taishan/taishankaifa2/taishanyanshi/songbai/fuduoduo），门控默认关、按租户 .env 开；taishan 真实钉钉端到端通过（建库→模板回执→worker 到点→卡片投递 success）。无 DB 变更（0016 已够）。sales_actions 68 单测 + loader/config 27 测试通过 + tz/合并回归测试。 |
| [2026-07-10](changelog/2026-07-10.md) | **钉钉卡片反馈采集（M1 capture-first）**：补上「卡片点赞/点踩/问题反馈多选点了收不到」的接收缺口。stream client 注册卡片回调 topic（`/v1.0/card/instances/callback`），新增 `SalesAgentCardCallbackHandler` 防御式采集 → 原始载荷落新表 `dingtalk_card_callback_events`（migration 0018）。**全租户优雅降级**：无条件注册、异常只 log 不外抛、恒 ACK，不发卡片的租户零副作用。归因免映射表（回调带 user_id，conversation_id 可确定性重算）。「原始载荷 → `feedbacks`」的映射（含点踩多选 → labels/categories）留待 M2 基于真实样本。4 单测 + 迁移 up/down 隔离验证通过。 |
