...
所有层 prompt（task / system / router / risk / coach / web）已统一纳入 DB 版本管理（26 个内置槽位，2026-07-07 补注册 `router/context_resolver` + `router/clarification_resolver` 并让三个 router prompt 运行时走 PromptRegistry，后台编辑即时生效；2026-07-06 迁入福多多旧 prompt 并新增 `web/web_analysis`）：
